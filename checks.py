...
def _row(label, value, status=STATUS_INFO, note=""):
...
def check_identity(adb: AdbSession) -> List[Dict]:
    ...
    rows.append(_row("Brand / Manufacturer", f"{brand.upper() if brand else 'N/A'} ({manufacturer or 'N/A'})", STATUS_GOOD))
    rows.append(_row("Model", model or "N/A", STATUS_GOOD))
    if marketing_name:
        rows.append(_row("Nama Marketing", marketing_name, STATUS_INFO))
    rows.append(_row("Codename Internal", device_codename or "N/A", STATUS_INFO))
    rows.append(_row("Serial Number", serial or "N/A", STATUS_GOOD if serial else STATUS_WARNING))
    rows.append(_row("Versi Android", f"{android_ver or 'N/A'} (SDK {sdk_level or 'N/A'})", STATUS_GOOD))
    ...
    if patch:
        ...
        rows.append(_row("Security Patch Level", patch, status, note))
    else:
        rows.append(_row("Security Patch Level", "N/A", STATUS_WARNING))

    rows.append(_row("Build ID", build_id or "N/A", STATUS_INFO))

    # Build type: user = release resmi, userdebug/eng = build developer (mencurigakan di unit retail)
    if build_type:
        if build_type == "user":
            rows.append(_row("Build Type", build_type, STATUS_GOOD, "Build resmi retail"))
        else:
            rows.append(_row("Build Type", build_type, STATUS_BAD, "Build developer/debug - tidak wajar di unit retail, indikasi custom firmware"))
    ...
    if brand and "samsung" in brand.lower():
        ...
        if csc:
            if csc.upper() == "XID":
                rows.append(_row("Kode Region/CSC", csc, STATUS_GOOD, "Resmi SEIN Indonesia"))
            else:
                rows.append(_row("Kode Region/CSC", csc, STATUS_WARNING, "Unit Inter/Non-SEIN - cek legalitas & garansi"))
    else:
        ...
        if region:
            rows.append(_row("Region/Country Code", region, STATUS_INFO))

    rows.append(_row("Build Fingerprint", fingerprint or "N/A", STATUS_INFO))
    return rows
...
def check_connectivity(adb: AdbSession) -> List[Dict]:
    ...
    if imei1:
        rows.append(_row("IMEI Slot 1", imei1, STATUS_GOOD))
    else:
        rows.append(_row("IMEI Slot 1", "Tidak terbaca otomatis", STATUS_WARNING,
                          "Android 10+ membatasi akses IMEI via ADB. Cek manual via *#06# atau Settings > About Phone."))
    rows.append(_row("IMEI Slot 2 (dual SIM)", "Cek manual", STATUS_INFO,
    ...
    rows.append(_row("Baseband/Modem Version", baseband or "N/A", STATUS_GOOD if baseband else STATUS_WARNING))

    wifi_mac = adb.shell_first_match([
        "cat /sys/class/net/wlan0/address",
        "settings get secure wifi_mac_address",
    ])
    rows.append(_row("WiFi MAC Address", wifi_mac or "N/A", STATUS_GOOD if wifi_mac else STATUS_WARNING))

    bt_mac = adb.shell_first_match([
        "settings get secure bluetooth_address",
        "getprop ro.boot.btmacaddr",
    ])
    rows.append(_row("Bluetooth MAC Address", bt_mac or "N/A", STATUS_INFO))

    sim_state = adb.shell_first_match([
        "getprop gsm.sim.state",
    ])
    if sim_state:
        rows.append(_row("Status SIM", sim_state, STATUS_INFO))
...
def check_cpu_ram(adb: AdbSession) -> List[Dict]:
    ...
    rows.append(_row("Chipset/Platform", f"{(hardware or 'N/A').upper()} / {platform or 'N/A'}", STATUS_GOOD))
    rows.append(_row("CPU ABI", abi or "N/A", STATUS_INFO))
    if cpu_cores and cpu_cores.strip().isdigit():
        rows.append(_row("Jumlah Core CPU", f"{cpu_cores.strip()} core", STATUS_GOOD))
    else:
        rows.append(_row("Jumlah Core CPU", "N/A", STATUS_WARNING))
    ...
    if mem_total:
        ...
        rows.append(_row("Kapasitas RAM", f"~{total_gb} GB{avail_txt}", STATUS_GOOD))
    else:
        rows.append(_row("Kapasitas RAM", "N/A", STATUS_WARNING))

    swap_info = adb.shell("cat /proc/swaps 2>/dev/null | tail -n +2")
    if swap_info:
        rows.append(_row("Swap/ZRAM", "Aktif", STATUS_INFO, swap_info.splitlines()[0] if swap_info else ""))
...
def check_storage(adb: AdbSession) -> List[Dict]:
    ...
    if len(parts) >= 4:
        try:
            ...
            rows.append(_row("Total Storage (approx.)", approx, STATUS_GOOD, f"Partisi /data: {total_gb} GB"))
            used_pct = (used_gb / total_gb * 100) if total_gb else 0
            status = STATUS_GOOD if used_pct < 90 else STATUS_WARNING
            rows.append(_row("Terpakai / Sisa", f"Terpakai {used_gb} GB | Sisa {free_gb} GB", status))
        except (ValueError, ZeroDivisionError):
            rows.append(_row("Storage Raw Info", df_data or "N/A", STATUS_WARNING))
    else:
        rows.append(_row("Storage Info", "N/A - gagal membaca df", STATUS_WARNING))
    return rows
...
def check_display(adb: AdbSession) -> List[Dict]:
    ...
    rows.append(_row("Resolusi Layar", res_match.group(1) if res_match else "N/A", STATUS_GOOD))
    rows.append(_row("Kerapatan (DPI)", f"{dpi_match.group(1)} DPI" if dpi_match else "N/A", STATUS_INFO))
    ...
    if high_refresh:
        ...
        rows.append(_row("Refresh Rate", f"{rate} Hz terdeteksi", STATUS_GOOD, "Pastikan opsi high refresh rate aktif di Settings"))
    else:
        rows.append(_row("Refresh Rate", "60 Hz / tidak terdeteksi tinggi", STATUS_INFO, "Sebagian device tidak expose info ini via ADB, cek manual di Settings > Display"))
    ...
    if panel_info:
        rows.append(_row("Info Panel (raw)", panel_info.splitlines()[0][:80], STATUS_INFO,
                          "Bandingkan manual dengan spek resmi jika ingin cek indikasi layar pernah diganti"))

    rows.append(_row("Cek Fisik Layar", "Manual", STATUS_INFO, "Dead pixel, burn-in, touch response harap dicek langsung"))
    return rows
...
def check_battery(adb: AdbSession) -> Dict:
    ...
    if level:
        ...
        rows.append(_row("Sisa Baterai", f"{lv}%", STATUS_GOOD))
    if present:
        rows.append(_row("Baterai Terdeteksi", present.group(1), STATUS_GOOD if present.group(1) == "true" else STATUS_BAD))
    if technology:
        rows.append(_row("Jenis Baterai", technology.group(1).strip(), STATUS_GOOD))

    if temp:
        temp_c = float(temp.group(1)) / 10.0
        status = STATUS_GOOD if temp_c < 38.0 else STATUS_WARNING if temp_c <= 42.0 else STATUS_BAD
        rows.append(_row("Suhu Baterai", f"{temp_c:.1f} °C", status))
        extra["batt_temp"] = temp_c

    if voltage:
        volt_v = float(voltage.group(1)) / 1000.0
        status = STATUS_GOOD if 3.6 <= volt_v <= 4.4 else STATUS_WARNING
        rows.append(_row("Tegangan Baterai", f"{volt_v:.2f} V", status))
    ...
    rows.append(_row("Status Kesehatan (sistem)", h_label, h_status))
    ...
    if cycle_raw:
        ...
        if m:
            ...
            rows.append(_row("Siklus Pengisian (Cycle Count)", f"{cycle_num}x", status))
            extra["cycle_count"] = cycle_num
    if cycle_num is None:
        rows.append(_row("Siklus Pengisian (Cycle Count)", "N/A", STATUS_INFO,
    ...
    if charge_full and charge_full_design:
        try:
            ...
            if design > 0:
                ...
                rows.append(_row("Estimasi Kesehatan Baterai (wear)", f"{wear_pct}% dari kapasitas awal", status,
                                  "Dihitung dari charge_full vs charge_full_design"))
                extra["battery_wear_pct"] = wear_pct
        except Exception:
            pass
    else:
        rows.append(_row("Estimasi Kesehatan Baterai (wear)", "N/A", STATUS_INFO,
...
def check_thermal(adb: AdbSession, batt_extra: Dict) -> Dict:
    ...
    rows.append(_row("Waktu Nyala (Uptime)", uptime or "N/A", STATUS_INFO))
    ...
    if boot_reason:
        ...
        rows.append(_row("Alasan Reboot Terakhir", boot_reason, status))
    ...
    if thermal_raw:
        ...
        if m:
            ...
            rows.append(_row("Suhu Core CPU", f"{cpu_temp:.1f} °C", status))
            extra["cpu_temp"] = cpu_temp

    if batt_extra.get("is_charging") and batt_extra.get("batt_temp", 0) > 38.0:
        rows.append(_row("Peringatan Suhu saat Cas", "Suhu tinggi saat charging", STATUS_BAD, "Indikasi dipakai berat sambil dicas"))
    else:
        rows.append(_row("Peringatan Suhu saat Cas", "Normal", STATUS_GOOD))
...
def check_hardware_sensors(adb: AdbSession) -> List[Dict]:
    ...
    if cpu_online:
        rows.append(_row("Core CPU Aktif", cpu_online, STATUS_GOOD, "Semua core seharusnya terdaftar aktif di sini"))
    ...
    if sensor_list:
        ...
        for s in expected:
            if s in low:
                found.append(s)
        rows.append(_row("Sensor Terdeteksi", f"{len(found)}/{len(expected)} sensor umum ditemukan ({', '.join(found) if found else '-'})",
        ...
        rows.append(_row("Sensor Terdeteksi", "N/A - gagal membaca sensorservice", STATUS_WARNING))
    ...
    if current_now:
        rows.append(_row("Arus PMIC (current_now)", current_now.strip()[:60], STATUS_INFO))
...
def check_system_integrity(adb: AdbSession, brand: str) -> List[Dict]:
    ...
    if bootstate:
        ...
        rows.append(_row("Verified Boot State", bootstate, status,
        ...
        rows.append(_row("Bootloader Lock State", "Locked" if flash_locked == "1" else "Unlocked", status))
    else:
        rows.append(_row("Status Bootloader", "N/A - tidak terbaca", STATUS_WARNING))

    if tags:
        status = STATUS_GOOD if "release-keys" in tags else STATUS_BAD
        rows.append(_row("Build Tags", tags, status,
    ...
    if brand and "samsung" in brand.lower():
        ...
        if knox:
            if knox.strip() == "0":
                rows.append(_row("Samsung Knox Warranty Bit", "0x0", STATUS_GOOD, "Pabrikan murni, fitur Knox/Samsung Pass utuh"))
            else:
                rows.append(_row("Samsung Knox Warranty Bit", f"0x{knox.strip()}", STATUS_BAD, "Knox tripped - pernah root/custom, sebagian fitur mati permanen"))
    ...
    if su_check or magisk_check:
        rows.append(_row("Deteksi Root", "Terindikasi rooted (su binary/Magisk ditemukan)", STATUS_BAD))
    else:
        rows.append(_row("Deteksi Root", "Tidak terdeteksi root", STATUS_GOOD))
...
def check_originality(adb: AdbSession, batt_extra: Dict) -> List[Dict]:
    ...
    if brand and manufacturer:
        if brand not in fingerprint and manufacturer not in fingerprint:
            rows.append(_row("Konsistensi Brand vs Fingerprint", "Tidak cocok", STATUS_BAD,
            ...
            rows.append(_row("Konsistensi Brand vs Fingerprint", "Cocok", STATUS_GOOD))
    ...
    first_api = adb.getprop("ro.product.first_api_level") or adb.getprop("ro.build.version.sdk")
    if cycle is not None and wear is not None:
        if cycle < 30 and wear < 90:
            rows.append(_row("Indikasi Baterai Diganti", "Mencurigakan", STATUS_WARNING,
            ...
            rows.append(_row("Indikasi Baterai Diganti", "Kemungkinan baterai baru/original rendah pemakaian", STATUS_INFO))
        else:
            rows.append(_row("Indikasi Baterai Diganti", "Tidak ada anomali signifikan", STATUS_GOOD))
    else:
        rows.append(_row("Indikasi Baterai Diganti", "Data tidak cukup untuk dianalisis", STATUS_INFO))
    ...
    if camera_info:
        n_cam = len(re.findall(r"camera id", camera_info.lower()))
        rows.append(_row("Modul Kamera Terdeteksi", camera_info.splitlines()[0][:80] if camera_info else "N/A", STATUS_INFO,
    ...
    rows.append(_row("Catatan Originalitas", "Hasil di atas adalah indikasi awal berbasis data sistem",
                      STATUS_INFO,
